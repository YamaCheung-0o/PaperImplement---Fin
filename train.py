...
def main():
    ...
    # if all_train_labels.shape[1] > 1:  # 多分类
    #     train_label_dist = np.sum(all_train_labels, axis=0)
    #     print("训练集类别分布:", train_label_dist)
    # else:  # 二分类
    #     positive_count = np.sum(all_train_labels > 0)
    #     negative_count = len(all_train_labels) - positive_count
    #     print(f"训练集类别分布: 负样本={negative_count}, 正样本={positive_count}")
    #     print(f"正负样本比例: {positive_count/negative_count:.3f}:1")
    ...
    for epoch in range(cfg["epochs"]):
        ...
        # 添加预测分析
        if epoch == 0 or metrics['kappa'] == 0 or metrics['f1'] == 0:
        ...
        if metrics["final"] > best_final:
            ...
            torch.save({"epoch": epoch+1, "state_dict": model.state_dict(), "metrics": metrics, "cfg": cfg}, best_path)
            print(f"[Checkpoint] new best final={best_final:.3f} saved -> {best_path}")

    print(f"Best Final-score: {best_final:.3f} -> {best_path}")
